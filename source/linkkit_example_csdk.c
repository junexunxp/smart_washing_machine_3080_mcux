...
#define LIGHT_SWITCH_CTRL 	"LightSwitch"
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
	...
	if(strstr(request,LIGHT_SWITCH_CTRL)){
		if(strstr(request,"1")){
			USER_LED_ON();
...
int linkkit_csdk_main(void *paras)
{
...
/* Task priorities. */
#define linkkitcsdk_runTASKPRIORITY (3)
void linkkitcsdk_init(void ){
	xTaskCreate(linkkit_csdk_main, "linkkitcsdk_run", 2048 + 10, NULL, linkkitcsdk_runTASKPRIORITY, NULL);
}
