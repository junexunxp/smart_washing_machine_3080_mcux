 ...
 */
int LPUART_RTOS_Send(lpuart_rtos_handle_t *handle, const uint8_t *buffer, uint32_t length, uint32_t timeout)
{
    EventBits_t ev;
    ...
    if (pdFALSE == xSemaphoreTake(handle->txSemaphore, portMAX_DELAY))
    {
 ...
 */
int LPUART_RTOS_Receive(lpuart_rtos_handle_t *handle, uint8_t *buffer, uint32_t length, size_t *received, uint32_t timeout)
{
    EventBits_t ev;
    size_t n              = 0;
    int retval            = kStatus_Success;
